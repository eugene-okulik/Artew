...
from test_api_artew_mexx.conftest import unique_object_id
...
def test_update_object_put(unique_object_id, update_object_put):
    print(f"Тест test_update_object_put: используемый object_id: {unique_object_id}")
    update_body = {"name": "Update_PUT_Artew", "data": {"color": "black&white", "size": "XXXL"}}
    update_object_put.update_object_put(unique_object_id, update_body)
    update_object_put.check_that_status_is_200()


def test_update_object_patch(unique_object_id, update_object_patch):
    print(f"Тест test_update_object_patch: используемый object_id: {unique_object_id}")
    update_body = {"data": {"color": "black&white&RED", "size": "XXL"}}
    update_object_patch.update_object_patch(unique_object_id, update_body)
    update_object_patch.check_that_status_is_200()


def test_delete_object(unique_object_id, delete_object):
    print(f"Тест test_delete_object: используемый object_id: {unique_object_id}")
    delete_object.delete_object(unique_object_id)
